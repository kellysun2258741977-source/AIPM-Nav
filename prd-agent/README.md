...
1. **① 澄清需求（可多轮）** —— 先别急着写。Agent 主动追问最关键的不确定点：目标用户是谁、要解决的核心问题、上线后用什么指标判断成功、MVP 边界在哪。每个问题都附「为什么问」和快捷选项。**若你的回答仍含糊（比如成功指标没给数字），它会再追问一轮**把指标钉成可量化目标，再往下走。
...
- **思考**：模型用自然语言解释「现在该做什么、为什么」，**逐 token 流式呈现**（真实模型为 SSE 流，演示模式为模拟 token 流，共用同一 UI）。
...
- 把澄清做成真正的多轮自由对话（当前为「触发式追问」，最多两轮）；
- 真实模型支持中断/重试，并对 token 用量与耗时做可视化。

> 已实现：可见 ReAct 循环、流式思考、触发式多轮澄清、零配置演示 + BYOK、Markdown 导出、深浅主题。从同仓库[导航页](../) 顶部的 **Featured** 卡片可一键进入。
